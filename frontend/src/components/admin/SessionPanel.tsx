...
export default function SessionPanel({ session: f, onClose, onUpdated, onDeleted }: Props) {
    ...
    return (
        <div className="flex flex-col md:h-full bg-wood-light rounded-xl shadow-md overflow-hidden animate-modal-in">
            ...
            <div className="px-6 pt-6 pb-4">
                ...
                <div className="flex flex-row gap-8 mt-2">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="font-didot text-xs text-wood-accent/70">{fmt(f.start_time)} – {fmt(f.end_time)} · {duration(f.start_time, f.end_time)}</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <svg className="w-3.5 h-3.5 shrink-0 text-wood-accent/50" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="flex gap-2 px-6 pb-4">
                <button
                    ...
                    className="flex-1 font-didot text-xs tracking-wide bg-wood-accent text-white hover:bg-wood-dark py-2 rounded-lg transition-colors duration-200"
                >
                    Edit Flow
                </button>
                <button
                    onClick={() => { if (clients.length > 0) { setCancelError(true); } else { setCancelError(false); setShowCancel(true); } }}
                    className="flex-1 font-didot text-xs tracking-wide bg-red-400 text-white hover:bg-red-600 py-2 rounded-lg transition-colors duration-200"
                >
                    Cancel Flow
                </button>
            </div>
                ...
                {clients.map(c => (
                    <div key={c.id} className="flex flex-row items-center justify-center py-2.5 border-b border-wood-accent/10 last:border-0 group">
                        ...
                        <button
                            ...
                            className="ml-3 shrink-0 bg-red-400 text-white hover:bg-red-600 rounded-xl p-2 transition-colors duration-200"
                        >
            ...
            <div className="px-6 py-4 border-t border-wood-accent/10">
                <button className="w-full font-didot text-xs tracking-wide bg-wood-accent text-white hover:bg-wood-dark py-2.5 rounded-lg transition-colors duration-200">
                    + Add Client
                </button>
            </div>
